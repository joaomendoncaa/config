...
Rectangle {
    ...
    Rectangle {
        ...
        Text {
            ...
            anchors.centerIn: parent
            text: root.upgrading ? "\uf110" : root.updateCount
    ...
    function enqueueUpgrade(pkgName, source) {
        for (var i = 0; i < root.upgradeQueue.length; i++) {
            if (root.upgradeQueue[i].name === pkgName) return
        }
        root.upgradeQueue.push({ name: pkgName, source: source })
        processQueue()
    }
    ...
    function enqueueUpgradeAll() {
        ...
        for (var i = 0; i < root.packages.length; i++) {
            ...
            if (!dup) {
                root.upgradeQueue.push({ name: root.packages[i].name, source: root.packages[i].source })
            }
        }
        processQueue()
    }
    ...
    function processQueue() {
        if (root.upgrading || root.upgradeQueue.length === 0) return
