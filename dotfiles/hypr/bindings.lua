...
hl.bind("SUPER + SHIFT + L", hl.dsp.exec_cmd("loginctl lock-session & omarchy-lock-screen"), { description = "Lock Screen" })
hl.bind("SUPER + ALT + RETURN", hl.dsp.exec_cmd('uwsm-app -- xdg-terminal-exec --dir="$(omarchy-cmd-terminal-cwd)" tmux attach'), { description = "Tmux" })
hl.bind("ALT + PRINT", hl.dsp.exec_cmd("$HOME/.config.jmmm.sh/bin/toggle-record-screen"), { description = "Record Screen" })
hl.bind("SUPER + SHIFT + R", hl.dsp.exec_cmd("$HOME/.config.jmmm.sh/bin/toggle-obs-record"), { description = "Toggle OBS Record" })
...
hl.bind("SUPER + SHIFT + B", hl.dsp.exec_cmd("omarchy-launch-webapp 'http://localhost:3000'"), { description = "Development Browser" })
hl.bind("SUPER + SHIFT + W", hl.dsp.exec_cmd("steam steam://rungameid/230410"), { description = "Launch Warframe" })
hl.bind("SUPER + SHIFT + A", hl.dsp.exec_cmd('omarchy-launch-webapp "https://chatgpt.com"'), { description = "Launch ChatGPT" })
hl.bind("SUPER + SHIFT + T", hl.dsp.exec_cmd('omarchy-launch-webapp "https://translate.google.pt/?sl=auto&tl=en&op=translate"'), { description = "Launch Translator" })
hl.bind("SUPER + SPACE", hl.dsp.exec_cmd([[bash -c 'pgrep -x quickshell >/dev/null && quickshell ipc -p ~/.config/quickshell call launcher toggle || (quickshell & sleep 1 && quickshell ipc -p ~/.config/quickshell call launcher toggle)']]), { description = "Open Launcher" })
hl.bind("SUPER + SHIFT + SPACE", hl.dsp.exec_cmd([[bash -c 'if pgrep -x quickshell >/dev/null; then pkill -x quickshell; else quickshell & fi']]), { description = "Toggle Quickshell" })
...
hl.define_submap("toggles", "reset", function()
    hl.bind("T", function()
        hl.dispatch(hl.dsp.exec_cmd([[bash -c 'if [ "$(omarchy-theme-current)" = "Snow" ]; then omarchy-theme-set mars; else omarchy-theme-set snow; fi; ~/.config.jmmm.sh/bin/nvim-theme-sync']]))
...
hl.define_submap("ai", "reset", function()
    hl.bind("A", function()
        hl.dispatch(hl.dsp.exec_cmd('omarchy-launch-webapp "https://chatgpt.com" & omarchy-launch-webapp "https://grok.com" & omarchy-launch-webapp "https://www.perplexity.ai"'))
        hl.dispatch(hl.dsp.submap("reset"))
