...
export default {
    ...
    async setup(ctx) {
        ...
        function aggregateDiff(family) {
            ...
            for (const session of family) {
                for (const diff of session?.revert?.files ?? []) {
                    if (!diff?.file)
                        continue
                    const previous = files.get(diff.file) || { additions: 0, deletions: 0 }
                    files.set(diff.file, {
        ...
        function deriveState(root, family) {
            ...
            const running = family.some(session => data.session.status(session.id) === 'running')
            if (running && !runtime.busy)
                runtime.error = false
            runtime.busy = running
            const errored = family.some(session => erroredBySession.has(session.id))
            const blocked = errored || family.some(session => pendingCount(session.id) > 0)
        ...
        function degradedRecord(route, selectedSessionID) {
            const record = {
                ...
                route: route.type === 'session' ? 'session' : route.type || 'home'
            }
            if (selectedSessionID)
                record.selectedSessionID = selectedSessionID
            return record
        }
        ...
        function update() {
            ...
            const selectedSessionID = route.type === 'session' ? route.sessionID : ''
            if (!selectedSessionID) {
            ...
            if (!selected) {
                void data.session.sync(selectedSessionID, { children: true }).catch(() => {})
            ...
            const family = data.session.family(selectedSessionID)
        ...
        const unsubscribes = [
            ...
            'form.replied',
            'form.cancelled'
        ].map(type => data.on(type, event => {
            const sessionID = event?.data?.sessionID || event?.data?.form?.sessionID
            if (type === 'session.execution.failed' || type === 'session.step.failed' || type === 'session.retry.scheduled') {
                if (sessionID)
                    erroredBySession.add(sessionID)
            } else if (type === 'session.execution.started' || type === 'session.execution.interrupted') {
                if (sessionID)
                    erroredBySession.delete(sessionID)
            }
            if (sessionID)
